...
from rl_agent import StateKey

# Bin edges for `_bucket_linear` (4 bins each: index 0..3 below highest edge, 4 at top).
PAIR_RANK_EDGES: Tuple[float, ...] = (5.0, 8.0, 11.0)
NONPAIR_HIGH_CARD_EDGES: Tuple[float, ...] = (6.0, 8.0, 11.0)
NONPAIR_LOW_CARD_EDGES: Tuple[float, ...] = (5.0, 8.0)
MY_STACK_BB_EDGES: Tuple[float, ...] = (15.0, 40.0, 100.0)
OPP_STACK_BB_EDGES: Tuple[float, ...] = (15.0, 40.0, 100.0)
POT_BB_EDGES: Tuple[float, ...] = (2.0, 8.0, 24.0)
TO_CALL_BB_EDGES: Tuple[float, ...] = (1.0, 3.0, 8.0)
...
def _encode_hole_pair(c0: Card, c1: Card) -> Tuple:
    ...
    if r0 == r1:
        ...
        return ("pair", _bucket_linear(float(r0), PAIR_RANK_EDGES))
    hi, lo = max(r0, r1), min(r0, r1)
    suited = 1 if c0.suit == c1.suit else 0
    # Legacy/fine non-pair bins: hi(4) x lo(3) x suited(2)
    return (
        "np",
        _bucket_linear(float(hi), NONPAIR_HIGH_CARD_EDGES),
        _bucket_linear(float(lo), NONPAIR_LOW_CARD_EDGES),
        suited,
    )
...
def encode_from_hand_state(state: HandState, hero: int) -> StateKey:
    """
    ...
    sb = _bucket_linear(my_stack_bb, MY_STACK_BB_EDGES)
    ob = _bucket_linear(opp_stack_bb, OPP_STACK_BB_EDGES)
    pb = _bucket_linear(pot_bb, POT_BB_EDGES)
    tb = _bucket_linear(tc_bb, TO_CALL_BB_EDGES)
