"""
Unified bot policies for the web UI: random baseline, Module 1+2 (A* + logic), Module 3 (MC).
"""
...
_BOT_AGENTS = frozenset({"random", "m12", "m3"})
...
def pick_bot_action(agent: Optional[str], state: HandState, rng: random.Random) -> Dict[str, Any]:
    """Dispatch to random / Module 1+2 / Module 3 Monte Carlo."""
    a = normalize_agent(agent)
    if a == "random":
        return random_legal_action(state, rng)
    if a == "m12":
        try:
            return _m12_action(state, rng)
        except Exception:
            return random_legal_action(state, rng)
    # m3
